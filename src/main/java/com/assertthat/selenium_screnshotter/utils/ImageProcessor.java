...
public class ImageProcessor {
    ...
    public static BufferedImage highlight(BufferedImage sourceImage, int width, int height, int x, int y, Color color, int lineWidth){
        byte defaultLineWidth = 3;
        Graphics2D g = sourceImage.createGraphics();
        g.setPaint(color);
        g.setStroke(new BasicStroke(lineWidth==0?defaultLineWidth:lineWidth));
        g.drawRoundRect(x, y,width, height, ARCH_SIZE, ARCH_SIZE);
        g.dispose();
        return sourceImage;
    }
    ...
    public static BufferedImage getElement(BufferedImage sourceImage, int width, int height, int x, int y){
        return sourceImage.getSubimage(x,y,width,height);
    }

    public static BufferedImage blurArea(BufferedImage sourceImage, int width, int height, int x, int y){
        BufferedImage blurredImage =  blur(sourceImage.getSubimage(x,y,width,height));
        ...
        g.drawImage(blurredImage,x, y, null);
        g.dispose();
        return combined;
    }

    public static BufferedImage blurExceptArea(BufferedImage sourceImage, int width, int height, int x, int y){
